...
const SkinCare = () => {
  ...
  return (
    <>
      <Banner imageUrl={BgBanner} title={"Skin Care"} />
        ...
        <main className="w-[70%] mt-2">
          ...
          <div className="w-4/5 mx-auto flex justify-center gap-2 my-6">
            <button disabled={pages === 1} className="bg-gray-200 p-3 rounded disabled:opacity-50"
              onClick={() => selectPageHandler(pages - 1)} aria-label="Previous Page">
              <FaArrowAltCircleLeft />
            </button>
        
            {pageNumbers.map((num) => (
              <button key={num} aria-label="page number" className={`p-3 rounded 
              ${pages === num ? "bg-gray-400" : "bg-gray-100"}`} onClick={() => selectPageHandler(num)}>
                {num}
              </button>
            ))}
            
            <button disabled={pages === totalPages} className="bg-gray-200 p-3 rounded disabled:opacity-50"
              onClick={() => selectPageHandler(pages + 1)} aria-label="Next Page"
            >
